...
export async function configureGit(email = "action@github.com", name = "actions-user") {
    if (!EMAIL_REGEX.test(email))
        throw new SyntaxError("Invalid email syntax");
 ...
 * @throws ExecError
 */
export async function commitAndPush(files, message) {
    let errorAccumulator = 0;
 ...
 * @throws ExecError
 */
export async function cloneWiki(repo, host, cloneTo, clear = false) {
    let errorAccumulator = 0;
